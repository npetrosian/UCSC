...
void freeDigraph(Digraph* pG) {
  ...
  for(i = 1; i <= numberOfVertices; i++) {
    ...
    // let curr be the first node at index i of the reverseAdjacencyList array
    curr = (*pG)->reverseAdjacencyList[i] ;
...
void freeSCC(Digraph G){
  ...
  for(i = 0; i < MAX_ARRAY_SIZE; i++) {
    ...
    // let curr be the first node at index i of the SCCArray 
    curr = G->SCCArray[i] ;
...
// adds v to the reverse adjacency list of u, if that edge doesn't 
// already exist. If the edge does already exist, does nothing. Used when edges are entered or added. 
// returns 0 if (u,v) is a legal edge, and the edge didn't already exist. 
// returns 1 if (u,v) is a legal edge and the edge did already exist.
// returns -1 if (u,v) is not a legal edge.
int reverseAddEdge(Digraph G, int u, int v) {

    // We will allocate a node only when needed later in this subroutine.
    Node N;

    // have curr point to the u'th element of the adjacency array
    Node curr = G->reverseAdjacencyList[u];
    ...
    // Case 1: Index u is empty! 
    curr = G->reverseAdjacencyList[u];
    if(curr == NULL){
        N = newNode(v);
        N->next = NULL;
        N->prev = NULL;
        G->reverseAdjacencyList[u] = N;
        // printf("case 1:\n");
        return(0);
    }
...
// deletes v from the reverse adjacency list of u, if that edge exists.
// if the edge doesn't exist, does nothing. Used when edges are deleted.
// returns 0 if (u,v) is a legal edge, and the edge did already exist.
// returns 1 if (u,v) is a legal edge, and the edge didn't already exist.
// returns -1 if (u,v) is not a legal edge.
int reverseDeleteEdge(Digraph G, int u, int v){
  ...
  curr = G->reverseAdjacencyList[u];
    ...
    // if v is the first node in the reverse adjacency array
    if (curr->prev == NULL) {
        G->reverseAdjacencyList[u] = curr->next;
        if (curr->next != NULL) {
            curr->next->prev = NULL;
        }
        free(nodeToBeFreed);
    ...
    // if v is the last node in the reverse adjacency array
    if (curr->next == NULL) {
        if (curr->prev != NULL) {
            curr = curr->prev;
            curr->next = NULL;
...
// prints the reverse adjacency list in Digraph G in the same format as an input line, so all that 
// a client needs to do is a single call to PrintDigraph(). But it should not print out an edge twice. 
// Achieve this by only printing the edge for {j, k} when j < k. 
void reversePrintDigraph(FILE* out, Digraph G) {
    ...
    fprintf(out, "ReversePrintDigraph\n");
    fprintf(out, "%d", numVertices);
    for(i = 0; i <= numVertices; i++){
        if(G->reverseAdjacencyList[i] != NULL){
            //printf("xxxxxx %d\n", i);
            
            //myList = getNeighbors(G, i);
            curr = G->reverseAdjacencyList[i];
            while(curr != NULL){
...
void insertNodeSCCArray(Digraph G, int w){
  ...
  Node N;
  Node curr = G->SCCArray[G->SCCIndex]; 

  // Case 1: Index w is empty!
  if(curr == NULL){
     N = newNode(w);
     N->next = NULL;
     N->prev = NULL;
     G->SCCArray[G->SCCIndex] = N;
     return;
  }

  // Case 2: There's something there
  while(curr->next != NULL){
     curr = curr->next;
  }
  // at this point, curr is pointing to the last item
  N = newNode(w);
  N->next = NULL;
  N->prev = curr;
  curr->next = N;
...
// returns the number of vertices (including u) that are in the same SCC
...
int getNumSCCVertices(Digraph G, int u){
  ...
  if(u < 1 || u > G->numVertices){ // if not a legal edge
    return (-1);
  }
  ...
  // Traverse all indexes of SCCArray
  for(i = 0; i <= G->SCCIndex; i++){
    // point to the first node at index i of SCCArray
    curr = G->SCCArray[i];
    // set vertex count to 0
    count = 0;
    // set found flag to false
    uFound = false;
    // traverse all nodes at index i
    while(curr != NULL){
      // increment node count
      count++;
      // if our node curr matches u, set found flag to true
      if(curr->data == u){
        uFound = true;
      }
      // go to the next node
    ...
    // we have the total count of the nodes, and our flag is true
    if(uFound == true){
      // return the count
      return(count);
    }
  }
  // if we are still here, we did not find a matching node
  return(0);
}

// returns 1 if u and v are in the same SCC of G, and returns 0 if u and v
// are not in the same SCC of the current digraph. 
// A vertex is always in the same SCC as itself.
...
int inSameSCC(Digraph G, int u, int v){
  ...
  if(u < 1 || u > G->numVertices || v < 1 || v > G->numVertices){ // if not a legal edge
    return (-1);
  }
  ...
  for(i = 0; i <= G->SCCIndex; i++){
    ...
    while(curr != NULL){
      // increment node count
      count++;
      // if our node curr matches u and/or v, set found flags to true
      if(curr->data == u){
        uFound = true;
      }
      if(curr->data == v){
  ...
  // if we are still here, we did not find u and v in an SCC
  return(0);
}
