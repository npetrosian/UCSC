/*
 ...
 becomes brighter. When the user clicks inside the sun, it will have red concentric circles inside. When the user 
 clicks anywhere outside the sun, it will change back to its original color. The user can click anywhere on the 
 screen for the mysterious orbiting object to change its direction of rotation. 
 */

int mysteryObjectRotationAngle;
int rotationIncrement = 2;
boolean concentricCircles = false; 
...
void draw() {
  ...
  // for loop inside of an if-statement
  if (concentricCircles) {
    for (int i = 0; i < sunHeight; i+=5) { 
      noFill();
...
// Clicking on the Sun covers the sun with concentric circles using a for Loop per 
// the rubric.
// Clicking on the Sun reverses its orbiting objects direction using rotation and 
// reverse rotation per the rubric
void mousePressed() {
  if (dist(mouseX, mouseY, sunX, sunY) <= sunWidth/2) {
    sunRed = 255;
    sunGreen = 0;
    sunBlue = 0;
